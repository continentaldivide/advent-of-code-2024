...
const solution = async () => {
  ...
  const helperFunction = (num, coords, memo = {}) => {
    let score = 0;
    if (num === 9) {
      return 1;
    }
    ...
    adjacentCoords.forEach((newCoords) => {
      ...
      const inMemo = memo[`${newCoords[0]}.${newCoords[1]}`];
      if (oneHigher && !inMemo) {
        memo[`${newCoords[0]}.${newCoords[1]}`] = true;
        score += helperFunction(num + 1, newCoords, memo);
      }
    });
    return score;
  };
